...
public class StudentEntryController {
    ...
    private void submitEntry() {
        ...
        try {
            ...
            if (msg.contains("ACCESS BLOCKED")) {
                // Auto-timed-out yesterday; needs admin approval
                showAlert(AlertType.ERROR, "Access Blocked", msg);

            } else if (msg.contains("not yet open") || msg.contains("already closed")) {
                // Outside 6 AM – 10 PM operating hours
                showAlert(AlertType.WARNING, "Outside Operating Hours", msg);

            } else if (currentSelection == AttendanceAction.TIME_OUT &&
                    msg.toLowerCase().contains("time in found")) {
                showAlert(AlertType.WARNING, "Time In Required",
                        msg + "\nSwitching to Time In — please submit again.");
                currentSelection = AttendanceAction.TIME_IN;
                applySelectionStyles();
            } else {
                showAlert(AlertType.WARNING, "Cannot Log Entry", msg);
            }
